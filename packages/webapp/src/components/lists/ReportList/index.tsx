...
export const ReportList = wrap(
	memo(function ReportList({ title }: ReportListProps): JSX.Element {
		...
		const [reportFilterOption, setReportFilterOption] = useState<FilterOptions | undefined>(
			undefined
		)
		...
		const handleConfirmDelete = useCallback(async () => {
			...
			if (res) {
				...
				const selectedService = activeServices.current.find(
					(s) => s.id === recordToDelete.serviceId
				)
